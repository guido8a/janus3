...
//        println "get Precios string "+fecha+"  "+fecha.format('yyyy-MM-dd')
        def sql = "SELECT r1.item__id,(SELECT r2.rbpcpcun from rbpc r2 where r2.item__id=r1.item__id and " +
                "r2.rbpcfcha = max(r1.rbpcfcha) and r2.lgar__id=${lugar.id}) " +
                "from rbpc r1 where r1.item__id in (${itemsId}) and r1.lgar__id=${lugar.id} and " +
...
//            println "res "+r
    ...
    def actualizaOrden(volumen, tipo) {
        ...
        def vlob = VolumenesObra.findAll("from VolumenesObra where obra = ${volumen.obra} order by orden asc,id desc")
