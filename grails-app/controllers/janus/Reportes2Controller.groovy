...
//            eq("estado", "A")
    ...
    def reporteExcelComposicionPartes() {
        ...
        def extra2 = " (2) group by i.itemcdgo, i.itemnmbr, u.unddcdgo, v.voitpcun, v.voittrnp, v.voitpcun, " +
                "g.grpo__id, g.grpodscr " +
                "ORDER BY g.grpo__id ASC, i.itemcdgo"

        def extra3 = " (3) group by i.itemcdgo, i.itemnmbr, u.unddcdgo, v.voitpcun, v.voittrnp, v.voitpcun, " +
                "g.grpo__id, g.grpodscr " +
                "ORDER BY g.grpo__id ASC, i.itemcdgo"
...
//        println "sql: $sql"
        ...
        def row = 0
        WritableSheet sheet = workbook.createSheet('Composicion', 3)
        WritableSheet sheet1 = workbook.createSheet('Materiales', 0)
        WritableSheet sheet2 = workbook.createSheet('Mano de obra', 1)
        WritableSheet sheet3 = workbook.createSheet('Equipos', 2)
    ...
    def parteComposicion(sql, sheet, obra, tipo){
        ...
        label = new jxl.write.Label(1, 2, tipo == 1 ? "MATERIALES" : (tipo == 2 ? "MANO DE OBRA" : (tipo == 3 ? "EQUIPOS" : '')), times16format); sheet.addCell(label);
        ...
        label = new jxl.write.Label(1, 10, "FECHA: " + printFecha(obra?.fechaCreacionObra), times16format);
        sheet.addCell(label);
        label = new jxl.write.Label(1, 12, "FECHA ACT.PRECIOS: " + printFecha(obra?.fechaPreciosRubros), times16format);
        sheet.addCell(label);
...
//                    label = new jxl.write.Label(9, fila, it?.subpresupuesto ? it?.subpresupuesto?.toString() : ""); sheet.addCell(label);
                    fila++
                }
            ...
            if(tipo == 0){
                ...
                number = new jxl.write.Number(7, ultimaFila + 4, (obra?.valor ? obra?.valor - totalDirecto : 0)?.round(2) ?: 0); sheet.addCell(number);

                label = new jxl.write.Label(6, ultimaFila + 5, "TOTAL: ", times16format); sheet.addCell(label);
                number = new jxl.write.Number(7, ultimaFila + 5, obra?.valor?.round(2) ?: 0); sheet.addCell(number);
            }
        }
